...
parser.add_argument('--anneal', type=float, default=4,
                    help='annealing factor of the learning rate')
...
parser.add_argument('--vocab', type=str, required=True,
                    help='Model vocabulary. It should be a super-set of the '
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        if not best_val_loss or val_loss < best_val_loss:
            ...
            optim.param_groups[0]['lr'] /= args.anneal
...
n_parameters = sum(p.view(-1).size(0) for p in model.parameters())
# Save the meta data for this model
meta = {'args': args.__dict__, 'best_val_loss': best_val_loss, 'test_loss': test_loss,
        'nparameters': n_parameters}
with open('{}.meta'.format(args.save), 'w') as fmeta:
